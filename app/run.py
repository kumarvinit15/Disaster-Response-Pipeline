...
def index():
    ...
    msg_counts = df.iloc[:,4:].sum().sort_values(ascending=False)
    msg_names = list(msg_counts.index)
    # Category correlation matrix for heatmap
    cat_corr = df.iloc[:,4:].corr().values
